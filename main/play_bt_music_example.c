...
void app_main(void)
{
    ...
    ESP_LOGI(TAG, "[3.1] Create i2s stream to write data to DAC");
    esp_err_t ret = dac_i2s_enable();
    if(ret!=ESP_OK)ESP_LOGI(TAG, "[3.1.1] Failed to init DAC");
    dac_channel_t CHAN1=DAC_CHANNEL_1;
    dac_channel_t CHAN2=DAC_CHANNEL_2;
    dac_output_enable(CHAN1);
    dac_output_enable(CHAN2);
    ...
    while (1) {
        ...
        if ((msg.source_type == PERIPH_ID_TOUCH || msg.source_type == PERIPH_ID_BUTTON || msg.source_type == PERIPH_ID_ADC_BTN)
            ...
            else if ( (int) msg.data == 39){
                ESP_LOGI(TAG, "[ * ] touch tap event ffw");
                periph_bluetooth_fast_forward(bt_periph);
            }
            else if ( (int) msg.data == 36){
                ESP_LOGI(TAG, "[ * ] touch tap event rewind");
